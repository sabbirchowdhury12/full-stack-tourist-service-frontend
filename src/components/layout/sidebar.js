...
const Sidebar = ({ setExpand }) => {
  ...
  const sidebarItems = generateSidebarItems(user?.role);
  ...
  const generateIcon = (icon) => {
    ...
    icons_map["signout"] = (
      <svg
        ...
        fill="currentColor"
      >
        <path d="M224.51465,136.48535l-42,42a12.0001,12.0001,0,1,1-16.9707-16.9707L187.05859,140H104a12,12,0,0,1,0-24h83.05859L165.544,94.48535a12.0001,12.0001,0,1,1,16.9707-16.9707l42,42A11.99975,11.99975,0,0,1,224.51465,136.48535ZM120,204H52V52h68a12,12,0,0,0,0-24H48A20.02229,20.02229,0,0,0,28,48V208a20.02229,20.02229,0,0,0,20,20h72a12,12,0,0,0,0-24Z" />
      </svg>
    );
  ...
  return (
    <nav
      role="navigation"
      className={[
        "bg-primary text-white border-r border-slate-100 z-50 shadow-sm absolute inset-y-0 left-0",
    ...
    >
      <button
        className="absolute z-50 top-16 -right-3 bg-black hover:bg-slate-100 text-slate-500 p-0.5 rounded-full border border-slate-200"
        onClick={() => {
          ...
          <div className="text-slate-500">
            ...
            <div className="mt-3 mb-10 p-0">
              <ul className="list-none text-sm font-normal px-3">
                {sidebarItems?.map((item, index) => generateMenu(item, index))}
